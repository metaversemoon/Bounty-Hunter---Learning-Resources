...
function Create() {
  const { account, contract, myGaslessWallet } = useContext(MyAppContext)
  const dummyData = [
    {
  ...
  const SaveAllAndPublish = async () => {
    ...
    try {
      ...
      const obj = {
        ...
        questionsArray: data.length > 0 ? data : dummyData,
      }
      ...
      if (metadata) {
        ...
        let data = obj.encodeFunctionData('addTask', [
          fullUrl,
          rewardAmountInt,
          0,
        ])
        ...
        console.log('RELAY ID taskId:', taskId)
